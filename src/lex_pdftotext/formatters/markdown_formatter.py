...
class MarkdownFormatter:
    ...
    def format_with_index(
        ...
        """
        Format text with procedural piece index and cross-references.

        Args:
            text: Processed legal document text
            metadata: Pre-extracted metadata (optional)
            include_metadata_header: Whether to include metadata section

        Returns:
            Markdown with navigable index and anchored document pieces
        """
        # Extract metadata if not provided
        if metadata is None:
            metadata = self.metadata_parser.parse(text)
        ...
        # Index table
        index = self.index_generator.generate_full_index(metadata)
        ...
        if include_metadata_header:
            metadata_md = self.metadata_parser.format_metadata_as_markdown(metadata)
            if metadata_md:
                sections.append("## Metadados Gerais\n")
                sections.append(metadata_md)
                sections.append("\n---\n")

        # Document pieces with anchors
        sections.append("## Peças Processuais\n")

        # Insert document headers at each document ID position
        processed_text = self._insert_document_anchors(text, metadata)
        sections.append(processed_text)

        return "\n".join(sections)

    def _insert_document_anchors(self, text: str, metadata: DocumentMetadata) -> str:
        """Insert anchor headers at each document ID location."""
        if not metadata.document_positions:
            return text

        # Sort positions in reverse order to insert without offset issues
        positions = sorted(metadata.document_positions, key=lambda x: x["position"], reverse=True)

        result = text
        for pos in positions:
            doc_id = pos["id"]
            context = pos.get("context_before", "") + " " + pos.get("context_after", "")
            doc_type = self.index_generator.detect_type(context)

            header = self.index_generator.generate_document_header(
                doc_id=doc_id,
                doc_type=doc_type,
            )

            # Find the line containing this document ID and insert header before it
            ...
            # Insert header before the line
            result = result[:line_start] + header + "\n\n" + result[line_start:]
