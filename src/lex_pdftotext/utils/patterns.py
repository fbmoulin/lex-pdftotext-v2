...
class RegexPatterns:
    ...
    @staticmethod
    def extract_document_ids_with_positions(text: str, context_chars: int = 50) -> list[dict]:
        """Extract document IDs with position information.

        Args:
            text: Document text to search
            context_chars: Number of characters to capture before/after ID

        Returns:
            List of dicts with: id, line, position, context_before, context_after
        """
        results = []
        lines = text.split("\n")
        line_offsets = []
        ...
        for match in RegexPatterns.DOC_ID.finditer(text):
            doc_id = match.group(1)
            pos = match.start(1)  # Position of the captured group (the ID digits)
            ...
            context_before = text[start_ctx:pos].strip()
            context_after = text[match.end() : end_ctx].strip()
