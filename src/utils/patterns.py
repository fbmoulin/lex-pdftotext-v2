...
class RegexPatterns:
    ...
    SIGNATURE_DATE: Pattern = re.compile(
        r'assinado\s+eletronicamente\s+em\s+(\d{2}/\d{2}/\d{4})',
        re.IGNORECASE
    )
    ...
    # Case value
    CASE_VALUE: Pattern = re.compile(
        r'Valor\s+da\s+causa:\s*R\$\s*([\d.,]+)',
        re.IGNORECASE
    )

    # Court/Vara identification
    COURT_NAME: Pattern = re.compile(
        r'(\d+ª\s+Vara\s+[^/\n]+)',
        re.IGNORECASE
    )
    ...
    INITIAL_PETITION: Pattern = re.compile(
        r'PETIÇÃO\s+INICIAL|EXCELENTÍSSIMO\s+SENHOR',
        re.IGNORECASE
    )
    DECISION: Pattern = re.compile(
        r'DECISÃO|DESPACHO|SENTENÇA',
        re.IGNORECASE
    )
    CERTIFICATE: Pattern = re.compile(
        r'CERTID[ÃA]O|CERTIFICO',
        re.IGNORECASE
    )

    # Section headers (common in PJe documents)
    SECTION_HEADER: Pattern = re.compile(
        r'^([IVX]+|[A-Z])\s*[-–—.]\s*([A-ZÇÁÀÂÃÉÈÊÍÏÓÔÕÖÚÜÑ\s]+)$',
        re.MULTILINE
    )

    # Remove patterns (noise)
    PAGE_NUMBER: Pattern = re.compile(r'Página\s+\d+\s+de\s+\d+', re.IGNORECASE)
    SIGNATURE_URL: Pattern = re.compile(
        r'https?://[^\s]+',
        re.IGNORECASE
    )
    VERIFICATION_CODE: Pattern = re.compile(
        r'(?:Código\s+de\s+)?Verificação:\s*[\w\-]+',
        re.IGNORECASE
    )
    ...
    DOC_NUMBER_FOOTER: Pattern = re.compile(
        r'Número\s+do\s+documento:\s*\d+',
        re.IGNORECASE
    )
    ...
    PHONE_CONTACT: Pattern = re.compile(
        r'Telefone:\s*\(\d+\)\s*\d+',
        re.IGNORECASE
    )
    ...
    # Standalone CEP
    CEP_STANDALONE: Pattern = re.compile(
        r',?\s*(?:CEP:\s*)?[\d\-]+\s*-\s*ES',
        re.IGNORECASE
    )

    # Uppercase detection (for normalization)
    ALL_CAPS_LINE: Pattern = re.compile(
        r'^[A-ZÇÁÀÂÃÉÈÊÍÏÓÔÕÖÚÜÑ\s\d.,;:!?()\[\]{}/"\'–—-]+$'
    )
